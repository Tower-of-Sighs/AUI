...
public final class DialogWindow {
    ...
    private void create() {
        ...
        y = height > 0 ? Math.max(12, (vh - height) / 2) : 48;
        ...
        Element titleText = el("SPAN", "aui-dialog-title-text");
        titleText.setTextContent(options.title());
        ...
        Element close = el("BUTTON", options.closeClass());
        ...
        heading.addEventListener("mousedown", e -> begin(e, Mode.MOVE));
        heading.append(title);
        heading.append(controls);
        window.append(heading);
        content = el("DIV", options.contentClass());
        content.setAttribute("style", height > 0 ? "position:relative;flex:1;min-height:0;" : "position:relative;");
        ...
        overlay.addEventListener("mouseup", e -> mode = Mode.NONE);
    ...
    private void handle(Mode mode) {
        Element e = el("DIV", "aui-dialog-resize");
        e.setAttribute("style", "position:absolute;z-index:2;" + mode.handleStyle());
        e.addEventListener("mousedown", v -> begin(v, mode));
        window.append(e);
    }

    private void begin(Event event, Mode next) {
        if (maximized || !(event instanceof MouseEvent e)) return;
        mode = next;
        startX = e.clientX;
        startY = e.clientY;
        startLeft = x;
    ...
    private void move(Event event) {
        if (maximized || mode == Mode.NONE || !(event instanceof MouseEvent e)) return;
        double dx = e.clientX - startX, dy = e.clientY - startY;
        if (mode.move) {
            x = startLeft + dx;
            y = startTop + dy;
        }
        if (mode.e) {
            width = Math.max(360, startWidth + dx);
        }
        if (mode.s) {
    ...
    private void applyBounds() {
        String style = "position:absolute;left:" + px(x) + ";top:" + px(y) + ";width:" + px(width) + ";pointer-events:auto;";
        if (height > 0) style += "height:" + px(height) + ";display:flex;flex-direction:column;";
        window.setAttribute("class", options.windowClass() + (maximized ? " maximized" : ""));
        window.setAttribute("style", style);
    }
