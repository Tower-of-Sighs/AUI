    ...
    public static void readTransition(List<Transition.Change> changeList, Style originStyle) {
        ...
        while (iterator.hasNext()) {
            ...
            if (name.startsWith("filter-")) {
                switch (name) {
                    case "filter-blur":
                        filterStr.append(String.format("blur(%.2fpx) ", val));
                        break;
                    case "filter-brightness":
                        filterStr.append(String.format("brightness(%.2f) ", val));
                        break;
                    case "filter-grayscale":
                        filterStr.append(String.format("grayscale(%.2f) ", val));
                        break;
                    case "filter-invert":
                        filterStr.append(String.format("invert(%.2f) ", val));
                        break;
                    case "filter-hue-rotate":
                        filterStr.append(String.format("hue-rotate(%.2fdeg) ", val));
                        break;
                    case "filter-opacity":
                        filterStr.append(String.format("opacity(%.2f) ", val));
                        break;
                }
        ...
        if (StringUtils.isNotNullOrEmpty(filterStr)) {
