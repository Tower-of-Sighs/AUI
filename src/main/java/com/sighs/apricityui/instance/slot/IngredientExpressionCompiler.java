...
public final class IngredientExpressionCompiler {
    ...
    private static List<ItemStack> compilePipe(String expression, int maxCandidates) {
        ...
        for (String part : expression.split("\\|")) {
            ...
            if (normalized.startsWith("#")) {
                append(result, tagCandidates(parseTag(normalized), maxCandidates - result.size()), maxCandidates);
            } else {
                ItemStack stack = ItemStackExpressionCompiler.parse(normalized);
                if (!stack.isEmpty()) append(result, List.of(stack), maxCandidates);
            }
        }
    ...
    private static List<ItemStack> jsonCandidates(String expression, int maxCandidates) {
        try {
            ...
            LinkedHashMap<String, ItemStack> result = new LinkedHashMap<>();
            append(result, List.of(ingredient.getItems()), maxCandidates);
            return List.copyOf(result.values());
    ...
    private static ResourceLocation parseTag(String expression) {
        return expression == null || expression.length() < 2 ? null : ResourceLocation.tryParse(expression.substring(1));
    }
    ...
    private static List<ItemStack> buildTagCandidates(ResourceLocation id) {
        ...
        if (FURNACE_FUEL_TAG.equals(id)) {
            for (Item item : BuiltInRegistries.ITEM) {
                ...
                if (ForgeHooks.getBurnTime(stack, RecipeType.SMELTING) > 0) result.add(stack);
            ...
            for (Item item : BuiltInRegistries.ITEM) {
                ItemStack stack = new ItemStack(item);
                if (stack.is(key)) result.add(stack);
                if (result.size() >= MAX_CANDIDATES) break;
            }
        }
        result.sort(Comparator.comparing(stack -> String.valueOf(BuiltInRegistries.ITEM.getKey(stack.getItem()))));
