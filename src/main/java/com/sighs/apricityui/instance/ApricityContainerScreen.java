...
public class ApricityContainerScreen extends AbstractContainerScreen<ApricityContainerMenu> {
    ...
    public ApricityContainerScreen(String path) {
        this(ApricityContainerMenu.createClientOnly(Minecraft.getInstance().player.getInventory(), path),
                ...
                new TextComponent("ApricityScreen"));
    }
    ...
    protected void renderBg(@NotNull PoseStack poseStack, float partialTick, int mouseX, int mouseY) {
        ...
        drawBoundSlotItems(poseStack);
        drawUnboundSlotItems(poseStack);
    }
    ...
    private void drawBoundSlotItems(PoseStack poseStack) {
        ...
        for (net.minecraft.world.inventory.Slot slot : menu.slots) {
            ...
            if (slot == clicked && !draggingItem.isEmpty() && splitting && !renderStack.isEmpty()) {
                ...
                if (AbstractContainerMenu.canItemQuickReplace(slot, carried, true) && menu.canDragTo(slot)) {
                    ...
                    ItemStack previewStack = carried.copy();
                    AbstractContainerMenu.getQuickCraftSlotCount(quickCraftSlots, quickCraftingType, previewStack, existingCount);
                    int placeCount = previewStack.getCount();
            ...
            poseStack.pushPose();
            poseStack.translate(0.0D, 0.0D, 100.0D + visual.zIndex);
            if (drawQuickCraftGhost) {
                ...
                fill(poseStack, ghostX, ghostY, ghostX + ghostSize, ghostY + ghostSize, QUICK_CRAFT_GHOST_COLOR);
            }
            applyItemScaleTransform(poseStack, drawX, drawY, visual.iconScale);
            itemRenderer.blitOffset = 100.0F + visual.zIndex;
            itemRenderer.renderAndDecorateItem(renderStack, drawX, drawY, slot.x + slot.y * imageWidth);
    ...
    private void drawUnboundSlotItems(PoseStack poseStack) {
        // Keep bound-slot rendering specialized for container screens.
        // Unbound (virtual) slot items are rendered via the shared pass.
        ItemRender.renderUnboundSlotItems(poseStack, new ArrayList<>(unboundSlots));
    }

    private void drawSlotHoverTooltipByElement(PoseStack poseStack, int mouseX, int mouseY) {
        if (linkedDocument == null) return;
    ...
    private void drawDevToolsOverlay(PoseStack poseStack) {
        ...
        Base.drawDocument(poseStack, devToolsDocument);
    }
