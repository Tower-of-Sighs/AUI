...
public final class SlotDataBinder {
    ...
    private static void setItemState(Item item, ItemStack stack, String overlay, boolean ghost) {
        ...
        item.setDrivenState(new ItemRenderState(safe, overlay, ghost, false, ItemRenderContext.resolveCooldownProgress(safe)), Item.Source.MENU);
    }
    ...
    public void syncAllSlotPositions(int leftPos, int topPos, boolean force) {
        for (SlotBinding binding : bindings.values()) {
            ...
            int x = (int) Math.round(position.x) - leftPos;
            int y = (int) Math.round(position.y) - topPos;
    ...
    public boolean shouldRebindSlotsFromDom(Document document) {
        return document != null && (generation != document.getRefreshGeneration() || !Objects.equals(fingerprint, fingerprint(document)));
    }

    public int findOperableSlotIndexAt(double mouseX, double mouseY) {
        for (SlotBinding binding : bindings.values())
            if (binding.item.canOperateBoundMenuSlot() && binding.item.containsItemPoint(mouseX, mouseY))
    ...
    public boolean isSlotBound(net.minecraft.world.inventory.Slot slot) {
        return bindings.containsKey(menu.slots.indexOf(slot)); }
    ...
    public boolean isBoundElementHovered(net.minecraft.world.inventory.Slot slot, double mouseX, double mouseY) {
        ...
        return binding != null && binding.item.canOperateBoundMenuSlot() && binding.item.containsItemPoint(mouseX, mouseY);
    }
