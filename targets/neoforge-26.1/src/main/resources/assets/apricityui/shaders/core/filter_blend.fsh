...
// The blend mode is carried in the spare radius slot for this dedicated
// pipeline. The common bridge writes it through the same uniform API used by
// legacy ShaderInstance backends.
#define BlendMode FilterOpacity.w
...
void main() {
    vec4 sourceRaw = texture(Sampler0, texCoord0);
    vec4 backdropRaw = texture(Sampler1, texCoord0);
