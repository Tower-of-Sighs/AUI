...
const harnessPath = path.join(
  ...
  "apricity",
  "tests",
  "resource-browser-gray-text-border.html"
);
...
const match = domOutput.match(/BROWSER_GRAY_TEXT_BORDER_METRICS\s+({.*})/);
if (!match) {
  console.error("Browser gray text/border metrics payload not found.");
  process.exit(2);
}

const payload = JSON.parse(match[1]);
const screenshotPath = path.join(screenshotDir, "resource-browser-gray-text-border-1463x843.png");
...
console.log("BROWSER_GRAY_TEXT_BORDER_METRICS " + JSON.stringify(payload));
