param(
    ...
    [string]$AuiFinalMetricsLog = "run/resource-browser-font-raster-aui-source-outline-coverage-4x-row-clamp-quad-snap-y-texture-gutter-1-projection-alpha-export-last.log",
    [string]$AuiImage = "run/screenshots/aui/2026-07-15_18.36.36.png",
    [string]$AuiMaskDir = "run/font-raster-masks",
    [string]$OutLog = "run/resource-browser-font-edge-classification.log",
    [string]$SampleIds = "arial12White,arial13White",
    [string]$AuiSource = "outline-coverage-4x-row-clamp",
    [string]$AuiQuadMode = "snap-physical-y-texture-gutter-1",
...
function Read-AuiMetrics {
    ...
    if (-not $metrics.viewport) { throw "AUI font raster viewport not found in $Path" }
    return $metrics
}
...
function Format-ColumnSet {
    param([hashtable]$Columns, [int[]]$Keys)
    $parts = New-Object System.Collections.Generic.List[string]
    foreach ($key in ($Keys | Sort-Object -Unique)) {
        if ($Columns.ContainsKey($key)) {
            $col = $Columns[$key]
            $avg = if ($col.ink -eq 0) { 0.0 } else { [Math]::Round([double]$col.value / $col.ink, 6) }
            $parts.Add("x=$key,ink=$($col.ink),avg=$avg")
        } else {
            $parts.Add("x=$key,missing")
        }
    }
    return "[" + ($parts -join ";") + "]"
}
...
function Get-EdgeReport {
    param(
        ...
        [int]$SourceWidth,
        [hashtable]$FinalColumns,
        [int]$FinalWidth,
        [int]$Count
    )
    $left = 0..([Math]::Max(0, $Count - 1))
    $rightStart = [Math]::Max(0, $SourceWidth - $Count)
    $right = $rightStart..([Math]::Max($rightStart, $SourceWidth - 1))
    $leftMissing = @($left | Where-Object { $SourceColumns.ContainsKey($_) -and -not $FinalColumns.ContainsKey($_) })
    $rightMissing = @($right | Where-Object { $SourceColumns.ContainsKey($_) -and -not $FinalColumns.ContainsKey($_) })
    return [pscustomobject]@{
        left = $left
        right = $right
...
function Find-AuiMask {
    param([array]$Metadata, [object]$Sample, [string]$Source, [string]$QuadMode)
    ...
    $matches = $Metadata | Where-Object {
        ...
        $_.cacheKey -like "*quadTexture=$QuadMode*"
    }
...
try {
    ...
    $lines.Add("auiFinalLog=$AuiFinalMetricsLog auiImage=$AuiImage auiMaskDir=$AuiMaskDir auiSource=$AuiSource auiQuadMode=$AuiQuadMode thresholdDarkness=$GlyphDarknessThreshold edgeColumns=$EdgeColumns")
    ...
    foreach ($sourceId in (Convert-SampleList $SampleIds)) {
        ...
        if (-not ($browserFinal.samples.PSObject.Properties.Name -contains $finalId) -or -not $auiFinal.samples.ContainsKey($finalId)) {
            $lines.Add("sample=$sourceId finalId=$finalId missing final metrics")
            continue
        }

        $browserSourceStats = $sourceSample.physicalDpr.sourceAlpha
        $browserSourceColumns = Convert-RelativeColumns (Convert-BrowserProfile $browserSourceStats.columnProfile) ([int]$browserSourceStats.bounds.x)
        $browserFinalSample = $browserFinal.samples.$finalId
        $browserBg = Convert-CssColorToRgb $browserFinalSample.style.backgroundColor
        $browserFinalStats = Measure-FinalColumns $browserBitmap $browserFinalSample.rect $browserScaleX $browserScaleY $browserBg $GlyphDarknessThreshold
        $browserFinalColumns = Convert-RelativeColumns $browserFinalStats.columns ([int]$browserFinalStats.minX)
        $browserReport = Get-EdgeReport $browserSourceColumns ([int]$browserSourceStats.bounds.width) $browserFinalColumns $browserFinalStats.width $EdgeColumns

        $auiMeta = Find-AuiMask $metadata $sourceSample $AuiSource $AuiQuadMode
        if (-not $auiMeta) {
            $lines.Add("sample=$sourceId missing AUI mask source=$AuiSource quadMode=$AuiQuadMode")
            continue
        }
        $auiSourceStats = Measure-AlphaColumns (Resolve-MaskPath $auiMeta.mask)
        $auiSourceColumns = Convert-RelativeColumns $auiSourceStats.columns ([int]$auiSourceStats.minX)
        $auiFinalSample = $auiFinal.samples[$finalId]
        $auiBg = Convert-CssColorToRgb $auiFinalSample.backgroundColor
        $auiFinalStats = Measure-FinalColumns $auiBitmap $auiFinalSample.rect $auiScaleX $auiScaleY $auiBg $GlyphDarknessThreshold
        $auiFinalColumns = Convert-RelativeColumns $auiFinalStats.columns ([int]$auiFinalStats.minX)
        $auiReport = Get-EdgeReport $auiSourceColumns $auiSourceStats.width $auiFinalColumns $auiFinalStats.width $EdgeColumns

        $lines.Add("sample=$sourceId finalId=$finalId text='$($sourceSample.text)'")
        $lines.Add("browser widths source=$($browserSourceStats.bounds.width) final=$($browserFinalStats.width) leftMissing=$($browserReport.leftMissing.Count) rightMissing=$($browserReport.rightMissing.Count)")
        $lines.Add("browser leftSource=$(Format-ColumnSet $browserSourceColumns $browserReport.left) leftFinal=$(Format-ColumnSet $browserFinalColumns $browserReport.left) leftMissing=[$($browserReport.leftMissing -join ',')]")
        $lines.Add("browser rightSource=$(Format-ColumnSet $browserSourceColumns $browserReport.right) rightFinal=$(Format-ColumnSet $browserFinalColumns $browserReport.right) rightMissing=[$($browserReport.rightMissing -join ',')]")
        $lines.Add("aui widths source=$($auiSourceStats.width) final=$($auiFinalStats.width) leftMissing=$($auiReport.leftMissing.Count) rightMissing=$($auiReport.rightMissing.Count) mask=$(Resolve-MaskPath $auiMeta.mask)")
        $lines.Add("aui leftSource=$(Format-ColumnSet $auiSourceColumns $auiReport.left) leftFinal=$(Format-ColumnSet $auiFinalColumns $auiReport.left) leftMissing=[$($auiReport.leftMissing -join ',')]")
        $lines.Add("aui rightSource=$(Format-ColumnSet $auiSourceColumns $auiReport.right) rightFinal=$(Format-ColumnSet $auiFinalColumns $auiReport.right) rightMissing=[$($auiReport.rightMissing -join ',')]")
    }
