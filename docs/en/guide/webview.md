...
- **keyboard**: once the iframe is its document's focused element, `keydown`/`keyup` go to the page and are **swallowed**, so Minecraft hotkeys do not fire at the same time. Text only lands if an editable element **inside the page** holds DOM focus (click the page's input first), exactly as in a real browser;
- **there is no native key injection API**: keys are synthesised as **DOM events inside the page**, so a browser default action never happens implicitly and the framework applies it explicitly (backspace/delete/enter/arrows in inputs are handled); IME composition is delivered as already-committed text, with no composition process.
...
- browser data (cookies, localStorage) lives under `game directory/apricity/webview` and survives restarts;
...
**Typing does nothing after clicking the input**: read `focus=` in `Iframe.status()`. `no` means the iframe is not its document's focused element (so AUI never forwards the characters); `yes` yet still nothing lands means the page itself has no focused element.
