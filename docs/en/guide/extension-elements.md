...
- **Encoding is the capture bottleneck.** WebView2 only offers a lossless (PNG) and a lossy (JPEG) screenshot format; on the same heavy 900×700 page PNG costs a ~110 ms round trip (≈9 fps) against JPEG's ~31 ms (≈28–33 fps). The `capture` attribute defaults to **auto**: a page that is not changing uses the lossless codec (and byte-level de-duplication of the payload skips the decode, the copy and the upload entirely, so it is free), and as soon as frames keep changing it switches to the fast codec. Pin it with `capture="lossless"` or `capture="fast"`;
- frames are picked up in the **render phase** (once per rendered frame), so the embedded page is not capped by the 20 Hz logic tick. The capture loop pauses after two seconds without a draw, so a hidden page does not burn CPU;
- `CapturePreview` itself tops out around **30 fps** (a 15–30 ms round trip at best). Bypassing it with a raw pixel stream was **tried and does not work here**: Windows.Graphics.Capture (DWM composition frame push) starts and delivers frames, but every frame is a **single flat colour** — the WebView2 content is attached through `IDCompositionTarget` (`CreateTargetForHwnd`) and neither WGC nor `PrintWindow(PW_RENDERFULLCONTENT)` reports it. The code is kept in `native/webview/src/frame_stream.{h,cpp}`; at runtime it is attempted, found to be blind, disabled automatically and the codec path is used, so it costs nothing — and it would start working on its own if that presentation ever changes. Two measured addenda: (1) auto no longer *attempts* it (that wasted one blank capture per view), so pin it with `capture="stream"` to try; (2) 30 fps is **resolution-independent** — a quarter of the area (450×350, a 40 KB payload against 149 KB) moved 28 fps to 33, so shrinking the capture will not raise the frame rate; use `capture-scale="0.5"` to save CPU and bandwidth (the CSS viewport stays correct, the picture gets softer);
